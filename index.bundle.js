...
(function (root, factory) {
  if (true) {
    !(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(79)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (raphael) {
      // AMD
...
"use strict";
module.exports = JSON.parse('{"to":"À","object":"Objet","objectPlaceholder":"Votre objet ici ...","body":"Corps","bodyPlaceholder":"Votre corps de mail ici ...","testYourEmail":"Tester votre email","strongPointsTitle":"✅ Les points forts","pointsToReviewTitle":"🛠 Les points à améliorer","sectorTexts":{"zero":"Il y a des améliorations à faire. Corrigez votre mail et réessayez encore !","one":"Bien joué mais des améliorations sont encore possibles !","two":"Bravo !!"},"mailObject":"Objet de l\'email","noObject":"Vous n\'avez pas entré d\'objet d\'email !","mailBody":"Corps de l\'email","noBody":"Vous n\'avez pas entré de corps d\'email !","character":"caractère","paragraph":"paragraphe","specialCharacters":"caractères spéciaux","word":"mot","tests":{"bulletPoints":{"label":"Bullet points","advices":{"zero":"Vous n\'avez pas de bullet point, mettez-en !","one":"Vous n\'avez qu\'un seul bullet point, mettez-en plus !","two":"Vous avez plus d\'un bullet point, parfait !"}},"capitalFirst":{"label":"Pas de majuscule en début d\'objet","advices":{"zero":"Votre objet ne devrait pas commencer par une majuscule.","two":"Bravo, votre objet ne commence pas par une majuscule !"}},"characterCount":{"label":"Nombre de caractères","advices":{"zero":{"under":"Votre email ne contient pas assez de caractères. Essayez entre 300 et 400 caractères.","over":"Votre email contient trop de caractères. Essayez entre 300 et 400 caractères."},"one":"La longueur de votre email peut être optimisé. Essayez entre 300 et 400 caractères.","two":"Bravo, votre email a le bon nombre de caractères (entre 300 et 400) !"}},"paragraphCount":{"label":"Nombre de paragraphes","advices":{"zero":"Votre email doit contenir entre 3 et 7 paragraphes.","one":"Votre email contient un peu trop de paragraphes. Il doit en contenir entre 3 et 7.","two":"Bravo, votre email contient le bon nombre de paragraphes (entre 3 et 7),"}},"secondLineQuestion":{"label":"Pas de question en deuxième ligne","advices":{"zero":"La deuxième ligne de votre email ne devrait pas être une question.","two":"Pas de question en deuxième ligne, bravo !"}},"specialCharacter":{"label":"Caractères spéciaux","advices":{"zero":"Trop de caractères spéciaux dans l\'objet ! L\'idéal est d\'en utiliser aucun.","one":"Un caractère spécial utilisé ! L\'idéal est d\'en utiliser aucun.","two":"Aucun caractère spécial utilisé, parfait !"}},"wordCount":{"label":"Nombre de mots","advices":{"zero":{"zero":"Pas de mot dans l\'objet !","over":"Le nombre de mots utilisé est trop élevé, essayez plutôt entre 1 et 4 mots !"},"one":"Le nombre de mots utilisé est un peu trop élevé, essayez plutôt entre 1 et 4 mots !","two":"Parfait, la longueur de votre objet est optimisée (entre 1 et 4 mots) ! "}}}}');
...
// EXTERNAL MODULE: ./node_modules/justgage/index.js
var justgage = __webpack_require__(628);
var justgage_default = /*#__PURE__*/__webpack_require__.n(justgage);
;// CONCATENATED MODULE: ./src/tests/word-count.js
const wordings = __webpack_require__(106);
...
;// CONCATENATED MODULE: ./src/tests/special-character.js
const special_character_wordings = __webpack_require__(106);
...
const specialCharacter = (input) => {
  ...
  if (count > 1) {
    return {
      score: 0,
      label: special_character_wordings.tests.specialCharacter.label,
      advice: `${count} ${special_character_wordings.specialCharacters}. ${special_character_wordings.tests.specialCharacter.advices.zero}`
    };
  } else if (count === 0) {
    return {
      score: 2,
      label: special_character_wordings.tests.specialCharacter.label,
      advice: special_character_wordings.tests.specialCharacter.advices.two
    };
  } else if (count === 1) {
    return {
      score: 1,
      label: special_character_wordings.tests.specialCharacter.label,
      advice: special_character_wordings.tests.specialCharacter.advices.one
    };
  }
}
...
;// CONCATENATED MODULE: ./src/tests/capital-first.js
const capital_first_wordings = __webpack_require__(106);

const capitalFirst = (input) => {
  if (firstIsUppercase(input)) {
    return {
      score: 0,
      label: capital_first_wordings.tests.capitalFirst.label,
      advice: capital_first_wordings.tests.capitalFirst.advices.zero
    }
  } else {
    return {
      score: 2,
      label: capital_first_wordings.tests.capitalFirst.label,
      advice: capital_first_wordings.tests.capitalFirst.advices.two
    }
  }
}
...
;// CONCATENATED MODULE: ./src/tests/bullet-points.js
const bullet_points_wordings = __webpack_require__(106);
...
const bulletPoints = (input) => {
  ...
  if (count === 0) {
    return {
      score: 0,
      label: bullet_points_wordings.tests.bulletPoints.label,
      advice: bullet_points_wordings.tests.bulletPoints.advices.zero
    }
  } else if (count === 1) {
    return {
      score: 1,
      label: bullet_points_wordings.tests.bulletPoints.label,
      advice: bullet_points_wordings.tests.bulletPoints.advices.one
    }
  } else if (count > 1) {
    return {
      score: 2,
      label: bullet_points_wordings.tests.bulletPoints.label,
      advice: bullet_points_wordings.tests.bulletPoints.advices.two
    }
  }
}
...
;// CONCATENATED MODULE: ./src/tests/paragraph-count.js
const paragraph_count_wordings = __webpack_require__(106);
...
const paragraphCount = (input) => {
  ...
  if (count >= 3 && count <= 7) {
    return {
      score: 2,
      label: paragraph_count_wordings.tests.paragraphCount.label,
      advice: `${count} ${paragraph_count_wordings.paragraph}s. ${paragraph_count_wordings.tests.paragraphCount.advices.two}`
    };
  } else if (count > 7 && count < 10) {
    return {
      score: 1,
      label: paragraph_count_wordings.tests.paragraphCount.label,
      advice: `${count} ${paragraph_count_wordings.paragraph}s. ${paragraph_count_wordings.tests.paragraphCount.advices.one}`
    };
  } else if (count > 10 || count < 3) {
    return {
      score: 0,
      label: paragraph_count_wordings.tests.paragraphCount.label,
      advice: `${count} ${paragraph_count_wordings.paragraph}${count < 2 ? '' : 's'}. ${paragraph_count_wordings.tests.paragraphCount.advices.zero}`
    };
  }
}
...
;// CONCATENATED MODULE: ./src/tests/second-line-question.js
const second_line_question_wordings = __webpack_require__(106);
...
const secondLineQuestion = (input) => {
  ...
  if (hasQuestionInSecondLine) {
    return {
      score: 0,
      label: second_line_question_wordings.tests.secondLineQuestion.label,
      advice: second_line_question_wordings.tests.secondLineQuestion.advices.zero
    };
  } else {
    return {
      score: 2,
      label:  second_line_question_wordings.tests.secondLineQuestion.label,
      advice: second_line_question_wordings.tests.secondLineQuestion.advices.two
    };
  }
}
...
;// CONCATENATED MODULE: ./src/tests/character-count.js
const character_count_wordings = __webpack_require__(106);
...
const characterCount = (input) => {
  ...
  if (count > 1000) {
    return {
      score: 0,
      label: character_count_wordings.tests.characterCount.label,
      advice: `${count} ${character_count_wordings.character}s. ${character_count_wordings.tests.characterCount.advices.zero.over}`
    };
  } else if (count < 300) {
    return {
      score: 0,
      label: character_count_wordings.tests.characterCount.label,
      advice: `${count} ${character_count_wordings.character}${count < 2 ? '' : 's'}. ${character_count_wordings.tests.characterCount.advices.zero.under}`
    };
  } else if (count > 300 && count < 400) {
    return {
      score: 2,
      label: character_count_wordings.tests.characterCount.label,
      advice: `${count} ${character_count_wordings.character}s. ${character_count_wordings.tests.characterCount.advices.two}`
    };
  } else {
    return {
      score: 1,
      label: character_count_wordings.tests.characterCount.label,
      advice: `${count} ${character_count_wordings.character}s. ${character_count_wordings.tests.characterCount.advices.one}`
    };
  }
}
...
;// CONCATENATED MODULE: ./src/scoring.js
...
const scoring_wordings = __webpack_require__(106);
class Scoring {
  ...
  static get sectors() {
    return {
      0: {
        ...
        color: "#FF0000",
        textColorClass: 'red',
        text: scoring_wordings.sectorTexts.zero,
      },
      1: {
        lo: 50,
        hi: 79,
        ...
        text: scoring_wordings.sectorTexts.one,
      },
      2: {
        lo: 80,
        hi: 100,
        ...
        text: scoring_wordings.sectorTexts.two,
      },
    }
  }
  ...
  get objectScoreDetails() {
    if (this.objectInput !== "") {
      ...
      return [{ score: 0, label: scoring_wordings.mailObject, advice: scoring_wordings.noObject }];
    }
  }
  ...
  get bodyScoreDetails() {
    if (this.bodyInput !== "") {
      ...
      return [{ score: 0, label: scoring_wordings.mailBody, advice: scoring_wordings.noBody }];
    }
  }
...
const src_wordings = __webpack_require__(106);
      ...
      <div class="container pt-5 d-flex justify-content-center">
        <div class="container-mail shadow">
          ...
          <div class="container-receiver-object-mail border-bottom">
            <div class="receiver p-2">${src_wordings.to} : emailchecker@meetyourmarket.fr</div>
            <div class="object p-2 w-100">${src_wordings.object} :
              <input class="object-input w-75" name="object" type="text" placeholder="${src_wordings.objectPlaceholder}"/>
            </div>
          </div>

          <div class="container-body-mail p-2">
            <textarea class="body-input" name="body" type="text-area" placeholder="${src_wordings.bodyPlaceholder}"></textarea>
          </div>
        </div>
      </div>

      <div class="container d-flex justify-content-center btn-container">
        <button class="btn-send shadow mt-4" id="btn-send" type="submit">💌 ${src_wordings.testYourEmail}</button>
      </div>
    </form>
  `);
}
...
function addScoreDetails(divDetailScore, scoring) {
  ...
  addPointsScoreDetails(divDetailScore, `${src_wordings.strongPointsTitle}`, testsGroupByScoreAndByType.strongPoints);
  addPointsScoreDetails(divDetailScore, `${src_wordings.pointsToReviewTitle}`, testsGroupByScoreAndByType.pointsToReview);
}
...
function addPointsScoreDetails(divDetailScore, title, groupedTests) {
  ...
  if (groupedTests.objectTests.length) {
    divDetailScore.insertAdjacentHTML('beforeend', scoreDetailsSubtitleHtml(src_wordings.object))
    groupedTests.objectTests.forEach(test => insertScoreDetail(divDetailScore, test));
  }

  if (groupedTests.bodyTests.length) {
    divDetailScore.insertAdjacentHTML('beforeend', scoreDetailsSubtitleHtml(src_wordings.body))
